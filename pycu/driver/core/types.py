...
class CuNDArrayType(Type):
		...
		if c_CuNDArray is None:
			class c_CuNDArray(ct.Structure):
				_fields_ = [('meminfo', ct.c_void_p),
							...
							('data', ct.c_uint64)] + \
							[(f'shape_{ax}', ct.c_ssize_t) for ax in range(ndim)] + \
							[(f'stride_{ax}', ct.c_ssize_t) for ax in range(ndim)]
				def __init__(self, cuNDarray):
					meminfo = 0
					parent = 0
					nitems = cuNDarray.size
					itemsize = cuNDarray.dtype.itemsize
					# data = ct.cast(cuNDarray.device_pointer.value, ct.POINTER(np.ctypeslib.as_ctypes_type(dtype))) # ct.c_void_p(arg.device_pointer.value)
					# print(cuNDarray.device_pointer, type(cuNDarray.device_pointer))
					data = cuNDarray.device_pointer
	...
	@staticmethod
	def construct(cuNDarray): #template, initialize
		#ISSUE: PASSING IN A NUMBA TYPE NOT A ctype OR A numpy DTYPE

		# np.ctypeslib.as_ctypes_type(self.dtype)

		#getattr(np, str(dtype))

		dtype = cuNDarray.dtype
		ndim = cuNDarray.ndim

		# if not isinstance(dtype, generic_ctype):
			# print("###")

		return CuNDArrayType(dtype, ndim)
