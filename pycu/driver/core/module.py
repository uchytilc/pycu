...
class Linker:
	def __init__(self, options = {}, *, handle = None, auto_free = True):
		if handle is None:
			optkeys, optvals, self._keep_alive = prepare_options(options)
			# del options
			handle = link_create(optkeys, optvals).value
		if auto_free:
...
# def _module_unload(handle):
# 	module_unload(handle)

class Module:
	def __init__(self, image, options = {}, *, handle = None, auto_free = True):
		if handle is None:
		...
		if auto_free:
			...
			# weakref.finalize(self, _module_unload, handle)

		self.handle = handle

	def __repr__(self):
		return f"Module() <{int(self)}>"
