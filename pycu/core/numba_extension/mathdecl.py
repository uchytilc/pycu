...
def determine_floating_type(typ, context):
    ...
    #note: 4 is enum for unsafe type conversion (i.e. loss of precision)
    if conversion == 4:
        rtrn = types.float64
        conversion = context.can_convert(typ, rtrn)
        if conversion == 4:
...
register_op("sign", unary_op_floating_type_factory)
...
#                 typ = typ_a
#             else:
#                 typ = self.context.unify_types(typ_a, typ_b)
#             return determine_floating_type(typ, self.context)
#         return typer
...
class BinaryFixedType(CallableTemplate):
    ...
    def generic(self):
        ...
        def typer(typ_a, typ_b):
            if not typ_a.is_internal or not typ_b.is_internal:
                return binary_external(typ_a, typ_b, op, self.context)
            return self.binary_rtrn
        return typer

def binary_op_fixed_type_factory(op, rtrn):
    @cuda_registry.register
...
register_op("mod", binary_op_floating_type_factory)
register_op("step", binary_op_floating_type_factory)
    ...
    def generic(self):
        ...
        def typer(typ_a, typ_b = None):
            ...
            if typ_b is not None:
                ...
                return self.context.unify_types(typ_a, typ_b)
            return typ_a
        return typer
...
def ternary_external(typ_a, typ_b, typ_c, op, context):
    try:
...
class TernaryFloatingType(CallableTemplate):
    def generic(self):
        ...
        def typer(typ_a, typ_b, typ_c):
            ...
            typ = self.context.unify_types(self.context.unify_types(typ_a, typ_b), typ_c)
            return determine_floating_type(typ, self.context)
        return typer
...
register_op("clamp", ternary_op_floating_type_factory)
register_op("mix", ternary_op_floating_type_factory)
register_op("lerp", ternary_op_floating_type_factory)
register_op("param", ternary_op_floating_type_factory)
register_op("smooth_step", ternary_op_floating_type_factory)
