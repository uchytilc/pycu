...
#note: stv = scalar to vector (converts a scalar to a vector containing the scalar for every entry)
def binary_op_factory_vec_stv(op, vec_t, op_helper):
	#converts scalar argument to a vector of the same length as the other argument
	@impl_registry.lower(op, vec_t, types.Number)
	def cuda_op_vec_stv(context, builder, sig, args):
		a = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[0])
		b = context.cast(builder, args[1], sig.args[1], sig.return_type.__member_type__)
		out = cgutils.create_struct_proxy(vec_t)(context, builder)
	...
	def cuda_op_stv_vec(context, builder, sig, args):
		a = context.cast(builder, args[0], sig.args[0], sig.return_type.__member_type__)
		b = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[1])
		out = cgutils.create_struct_proxy(vec_t)(context, builder)

		a_members = [a for attr in sig.return_type.__member_names__]
		b_members = [getattr(b, attr) for attr in sig.return_type.__member_names__]
...
def binary_op_factory(op, vec_t, op_helper):
	...
	binary_op_factory_vec_stv(op, vec_t, op_helper)
...
def binary_op_factory_vec_scalar(op, vec_t, op_helper, scalar_type = types.Number):
	...
	def cuda_op_vec_scalar(context, builder, sig, args):
		vec = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[0])
		scalar = args[1] #context.cast(builder, args[1], sig.args[1], scalar_type)
		out = cgutils.create_struct_proxy(vec_t)(context, builder)

		vec_members = [getattr(vec, attr) for attr in sig.return_type.__member_names__]

		caller = op_helper(context, builder, sig, args)
		return caller(out, vec_members, scalar)
...
def shift_impl(context, builder, out, vec, scalar, n, member_current):
	member_names = out._fe_type.__member_names__
	member_count = len(member_names)

	idx = context.get_constant(types.int32, np.int32(member_current))

	if member_current < member_count:
		with builder.if_else(builder.icmp_signed('==', n, idx)) as (true, false):
			with true:
				for m in range(member_count):
					setattr(out, member_names[(m + member_current)%member_count], vec[m])
			with false:
				shift_impl(context, builder, out, vec, scalar, n, member_current + 1)
	return out
...
def shift_helper(context, builder, sig, args):
	...
	def shift_caller(out, vec, scalar):
		#scalar.type
...
def ternary_op_factory_vec_vec_vec(op, vec_t, op_helper):
	...
	def cuda_op_vec_vec_vec(context, builder, sig, args):
		a = cgutils.create_struct_proxy(sig.return_type)(context, builder, value = args[0])
		b = cgutils.create_struct_proxy(sig.return_type)(context, builder, value = args[1])
		c = cgutils.create_struct_proxy(sig.return_type)(context, builder, value = args[2])
		out = cgutils.create_struct_proxy(vec_t)(context, builder)

		a_members = [getattr(a, attr) for attr in sig.return_type.__member_names__]
		b_members = [getattr(b, attr) for attr in sig.return_type.__member_names__]
		c_members = [getattr(c, attr) for attr in sig.return_type.__member_names__]

		op, caller = op_helper(context, builder, sig, args)
		return caller(op, builder, out, a_members, b_members, c_members)

#note: stv = scalar to vector (converts a scalar to a vector containing the scalar for every entry)
def ternary_op_factory_vec_vec_stv(op, vec_t, op_helper):
	@impl_registry.lower(op, vec_t, vec_t, types.Number)
	def cuda_op_vec_vec_stv(context, builder, sig, args):
		a = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[0])
		b = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[1])
		c = context.cast(builder, args[2], sig.args[2], sig.return_type.__member_type__)
		out = cgutils.create_struct_proxy(vec_t)(context, builder)

		a_members = [getattr(a, attr) for attr in sig.return_type.__member_names__]
		b_members = [getattr(b, attr) for attr in sig.return_type.__member_names__]
		c_members = [c for attr in sig.return_type.__member_names__]

		op, caller = op_helper(context, builder, sig, args)
		return caller(op, builder, out, a_members, b_members, c_members)

	@impl_registry.lower(op, vec_t, types.Number, vec_t)
	def cuda_op_vec_stv_vec(context, builder, sig, args):
		a = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[0])
		b = context.cast(builder, args[1], sig.args[1], sig.return_type.__member_type__)
		c = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[2])
		out = cgutils.create_struct_proxy(vec_t)(context, builder)
	...
	def cuda_op_stv_vec_vec(context, builder, sig, args):
		a = context.cast(builder, args[0], sig.args[0], sig.return_type.__member_type__)
		b = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[1])
		c = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[2])
		out = cgutils.create_struct_proxy(vec_t)(context, builder)
...
def ternary_op_factory_vec_stv_stv(op, vec_t, op_helper):
	@impl_registry.lower(op, vec_t, types.Number, types.Number)
	def cuda_op_vec_stv_stv(context, builder, sig, args):
		a = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[0])
		b = context.cast(builder, args[1], sig.args[1], sig.return_type.__member_type__)
		c = context.cast(builder, args[2], sig.args[2], sig.return_type.__member_type__)
		out = cgutils.create_struct_proxy(vec_t)(context, builder)
	...
	def cuda_op_stv_vec_stv(context, builder, sig, args):
		a = context.cast(builder, args[0], sig.args[0], sig.return_type.__member_type__)
		b = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[1])
		c = context.cast(builder, args[2], sig.args[2], sig.return_type.__member_type__)
		out = cgutils.create_struct_proxy(vec_t)(context, builder)
	...
	def cuda_op_stv_stv_vec(context, builder, sig, args):
		a = context.cast(builder, args[0], sig.args[0], sig.return_type.__member_type__)
		b = context.cast(builder, args[1], sig.args[1], sig.return_type.__member_type__)
		c = cgutils.create_struct_proxy(vec_t)(context, builder, value = args[2])
		out = cgutils.create_struct_proxy(vec_t)(context, builder)
...
def ternary_op_factory(op, vec_t, op_helper):
	ternary_op_factory_vec_vec_vec(op, vec_t, op_helper)
	ternary_op_factory_vec_vec_stv(op, vec_t, op_helper)
	ternary_op_factory_vec_stv_stv(op, vec_t, op_helper)

#########################
#ternary callers functions#
#########################

def clamp_caller(ops, builder, rtrn, *args):
	for n, attr in enumerate(rtrn._fe_type.__member_names__):
		setattr(rtrn, attr, op(builder, [arg[n] for arg in args]))
	return rtrn._getvalue()
...
def clamp_helper(context, builder, sig, args):
	pass
...
def vec_decl(vec, vec_t, Vec):
	...
	#SETATTR ONLY WORKS ON MUTABLE STRUCTS WHICH MUST BE STACK ALLOCATED
		#i.e. I need to return a pointer via cgutils.alloca_once() when creating the vector
		#vec = cgutils.create_struct_proxy(typ)(context, builder, ref = a_target_ptr)
			# the attributes will not be references if you use getattr()
			# use StructProxy._get_ptr_by_name to get a pointer to the attribute
...
def floating_vec_decl(vec, vec_t):
	...
	for scalar_type in [types.int32, types.int64]:
